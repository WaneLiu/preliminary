...
public class Producer {
    ...
    public static void main(String[] args) throws MQClientException, InterruptedException {
        ...
        producer.setNamesrvAddr("172.16.68.149:9876");
        producer.start();

        final String [] topics = new String[]{RaceConfig.MqTaobaoTradeTopic, RaceConfig.MqTmallTradeTopic};
        final Semaphore semaphore = new Semaphore(0);
        //此处信号量为0，后面订单发送成功则release一个，最后如果全部成功则release count个，
        //通过acquire(count)个判断是否全部成功，否则阻塞 jason

        for (int i = 0; i < count; i++) {
        	//每次产生一个订单和两个支付信息，分别发送到broker -jason
            try {
        ...
        producer.shutdown();
        System.out.println("producerFinish");
    }
}
